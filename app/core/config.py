...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = (  # pragma: allowlist secret
        "postgresql+asyncpg://vk_monitor:dev_password_123@localhost:5432/vk_monitor_dev"  # pragma: allowlist secret
    )
    DATABASE_ECHO: bool = False

    # Redis Configuration
    REDIS_URL: str = (
        "redis://:dev_redis_password@localhost:6379/0"  # pragma: allowlist secret
    )

    # Security Configuration
    SECRET_KEY: str = Field(
        default="vk-monitor-secret-key-change-in-production",  # pragma: allowlist secret
    ...
    VK_ACCESS_TOKEN: str = Field(
        default="", description="VK API access token - set via environment variable"
    )
    ...
    CELERY_BROKER_URL: str = (
        "redis://:dev_redis_password@localhost:6379/1"  # pragma: allowlist secret
    )
    CELERY_RESULT_BACKEND: str = (
        "redis://:dev_redis_password@localhost:6379/2"  # pragma: allowlist secret
    )
